...
module OpenApiTranspiler =
    ...
    let rec toOpenApiSchema (predicate: Lattice<Constraint>) : string * Fidelity =
        ...
        | Lattice.Leaf c ->
            match c with
            | IsNull -> """{"type": "null"}""", Fidelity.Exact
            | IsNotNull -> "{}", Fidelity.Exact
            | Range (Some (Exclusive v), None) -> $"""{{"exclusiveMinimum": {v}}}""", Fidelity.Exact
            | Range (None, Some (Exclusive v)) -> $"""{{"exclusiveMaximum": {v}}}""", Fidelity.Exact
            | Range (Some (Inclusive v), None) -> $"""{{"minimum": {v}}}""", Fidelity.Exact
            | Range (None, Some (Inclusive v)) -> $"""{{"maximum": {v}}}""", Fidelity.Exact
            | Range _ -> "{}", Fidelity.Approximate (Weaker "Complex range bounds not fully implemented in OpenAPI schema")
            | IntRange _ -> """{"type": "integer"}""", Fidelity.Approximate (Weaker "Int range requires precision bounds")
            | StringRange _ -> "{}", Fidelity.Approximate (Weaker "String range collation may differ and cannot be represented in OpenAPI")
            ...
            | RelativeBound(colA, op, colB) ->
                $"""{{"description": "Requires {colA} {op} {colB}"}}""", Fidelity.Approximate (Weaker "OpenAPI maps cross-field validation to description strings")
